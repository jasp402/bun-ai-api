...
export function renderWhatsAppQrHtml() {
  ...
  const imageTag = runtimeState.qrCode
    ? `<img src="data:image/png;base64,${runtimeState.qrCode}" alt="WhatsApp QR code" style="max-width: 360px; width: 100%;" />`
  ...
  return new Response(
    `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>WhatsApp QR</title></head><body style="font-family: sans-serif; padding: 24px;"><h1>WhatsApp QR</h1>${imageTag}${ascii}</body></html>`,
