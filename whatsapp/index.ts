...
export function renderWhatsAppQrHtml() {
  ...
  const imageTag = runtimeState.qrCode
    ? `<img src="data:image/png;base64,${runtimeState.qrCode}" alt="WhatsApp QR" style="max-width: 360px; width: 100%; margin: 0 auto; display: block;" />`
    : "<p style=\"text-align: center;\">No image QR available.</p>";

  const ascii = runtimeState.asciiQR
    ? `<pre aria-hidden="true" style="white-space: pre-wrap; font-size: 10px; line-height: 1; text-align: center; overflow-x: auto;">${escapeHtml(runtimeState.asciiQR)}</pre>`
    : "";
...
<body style="font-family: sans-serif; padding: 24px; max-width: 600px; margin: 0 auto; text-align: center;">
  <h1>WhatsApp QR</h1>
  ${imageTag}
  ${ascii}
</body>
