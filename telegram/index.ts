...
async function handleMessage(message: any) {
    ...
    if (!allowedUsersStr || allowedUsersStr.trim().length === 0) {
        console.warn(`[Security] Blocked access attempt from user ${userName || userIdStr} (${userIdStr}) because ALLOWED_TELEGRAM_USERS is not configured.`);
        await sendMessage(chatId, "⛔ Unauthorized: This bot is not configured to accept external connections.");
        return;
    }
